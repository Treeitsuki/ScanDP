# python scandp/convert_demos.py --demo_dir /home/cvl/cvl/ScanDP/save_dir --save_dir ./gridmap --save_img 1 --save_depth 1
...
def convert_dataset(args):
    ...
    for demo_file in demo_files:
        ...
        with h5py.File(file_name, "r") as data:
            ...
            action_array = data["pose"][:]
            gridmap_array = data["gridmap"][:]
            proprioception_array = data["pose"][:]
            ...
            gridmap_array = [gridmap_array[i] for i in range(length)]
            ...
            for i in range(length):
                ...
                if old_cloud.shape[0] > 10000:
                    ...
                    selected_idx = np.random.choice(
                        old_cloud.shape[0], 10000, replace=True)
            ...
            proprioception_array = [proprioception_array[i]
                                    for i in range(length)]
        ...
        gridmap_arrays.extend(gridmap_array)
    ...
    gridmap_arrays = np.stack(gridmap_arrays, axis=0)
    ...
    point_cloud_chunk_size = (
        single_size, cloud_arrays.shape[1], cloud_arrays.shape[2])
    action_chunk_size = (single_size, action_arrays.shape[1])
    gridmap_chunk_size = (
        gridmap_arrays.shape[1], gridmap_arrays.shape[2], gridmap_arrays.shape[3])
    if save_img:
        img_chunk_size = (
            single_size, color_arrays.shape[1], color_arrays.shape[2], color_arrays.shape[3])
        zarr_data.create_dataset('img', data=color_arrays, chunks=img_chunk_size,
                                 dtype='uint8', overwrite=True, compressor=compressor)

    if save_depth:
        depth_chunk_size = (
            single_size, depth_arrays.shape[1], depth_arrays.shape[2])
    ...
    zarr_data.create_dataset('state', data=state_arrays, chunks=state_chunk_size,
                             dtype='float32', overwrite=True, compressor=compressor)
    zarr_data.create_dataset('point_cloud', data=cloud_arrays, chunks=point_cloud_chunk_size,
                             dtype='float32', overwrite=True, compressor=compressor)
    zarr_data.create_dataset('action', data=action_arrays, chunks=action_chunk_size,
                             dtype='float32', overwrite=True, compressor=compressor)
    zarr_data.create_dataset('gridmap', data=gridmap_arrays, chunks=gridmap_chunk_size,
                             dtype='float32', overwrite=True, compressor=compressor)
    ...
    if save_img:
        cprint(
            f'color shape: {color_arrays.shape}, range: [{np.min(color_arrays)}, {np.max(color_arrays)}]', 'green')
    if save_depth:
        cprint(
            f'depth shape: {depth_arrays.shape}, range: [{np.min(depth_arrays)}, {np.max(depth_arrays)}]', 'green')
    cprint(
        f'cloud shape: {cloud_arrays.shape}, range: [{np.min(cloud_arrays)}, {np.max(cloud_arrays)}]', 'green')
    cprint(
        f'state shape: {state_arrays.shape}, range: [{np.min(state_arrays)}, {np.max(state_arrays)}]', 'green')
    cprint(
        f'action shape: {action_arrays.shape}, range: [{np.min(action_arrays)}, {np.max(action_arrays)}]', 'green')
