...
class PointNet2Encoder(nn.Module):
    ...
    def forward(self, xyz):
        ...
        xyz_reshaped = xyz.view(128, 1, -1)   # (128, 1, 25, 25, 25) → (128, 1, 15625)
        xyz = xyz_reshaped.expand(-1, 3, -1) 
        print(xyz.shape)
...
if __name__ == "__main__":
    ...
    model = PointNet2Encoder(in_channels=3, out_channels=256, use_bn=True).to(device)
    # print(model)
    xyz = torch.randn(128, 1, 8, 8, 8).to(device)
    
    # xyz = torch.randn(128, 3, 15625).to(device)
    t = time()
    out = model(xyz)
    print(out.size())
