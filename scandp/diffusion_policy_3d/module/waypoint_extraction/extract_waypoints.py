...
from diffusion_policy_3d.module.waypoint_extraction.traj_reconstruction import (
    pos_only_geometric_waypoint_trajectory,
    reconstruct_waypoint_trajectory,
    geometric_waypoint_trajectory,
)
...
if __name__ == "__main__":
    ...
    import torch
    import time
    # Generate test trajectory data (15 steps, 3D coordinates)
    np.random.seed(42)
    num_steps = 15
    dim = 3
    start = np.array([0.0, 0.0, 0.0])
    end = np.array([1.0, 1.0, 1.0])
    traj = np.linspace(start, end, num_steps) + 0.05 * np.random.randn(num_steps, dim)
    traj = torch.tensor(traj, device="cuda")

    # Measure the time taken for waypoint selection
    start_time = time.time()

    # Call the waypoint selection function
    waypoints = dp_waypoint_selection(
        actions=traj,
        gt_states=traj,  # Use the same data for simplicity
        err_threshold=0.005,  # Error threshold
        pos_only=True  # Use position only
    )

    end_time = time.time()
    elapsed_time = end_time - start_time
    print(f"Elapsed time for waypoint selection: {elapsed_time:.4f} seconds")

    # Calculate trajectory length
    def calculate_trajectory_length(points):
        return torch.sum(torch.norm(points[1:] - points[:-1], dim=1))

    original_length = calculate_trajectory_length(traj)
    waypoint_length = calculate_trajectory_length(traj[waypoints])
    ...
    # Plot waypoints
    ax.scatter(
        traj[waypoints, 0].cpu().numpy(),
        traj[waypoints, 1].cpu().numpy(),
        traj[waypoints, 2].cpu().numpy(),
        color="red",
        label="Viewpoints",
        s=50,
    )
    ...
    waypoint_traj = traj[waypoints]
    ax.plot(
        waypoint_traj[:, 0].cpu().numpy(),
        waypoint_traj[:, 1].cpu().numpy(),
        waypoint_traj[:, 2].cpu().numpy(),
        label="Optimized Trajectory",
        color="green",
        linestyle="--",
    )

    # Display trajectory lengths in the plot
    ax.text2D(
        0.05,
        0.95,
        f"Original Length: {original_length.item():.2f}\nViewpoint Length: {waypoint_length.item():.2f}",
