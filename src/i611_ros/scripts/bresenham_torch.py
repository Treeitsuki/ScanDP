...
def bresenham3D(pts_source, pts_target, map_size):
    if isinstance(map_size, list):
        assert len(
            map_size) == 3 and map_size[0] == map_size[1] == map_size[2], "map_size must be cubic"
        map_size = map_size[0]
    ...
    fractional_steps = step_indices / \
        (steps[:, None] - 1).clamp(min=1)  # 0.0 ~ 1.0 の割合
    ray_coords = source_pts[:, None, :] + (fractional_steps[..., None]
                                           * deltas[:, None, :]).round().int() * direction[:, None, :]

    # マスク適用して不要な部分を除去
    # **明示的に (N, 3) にリシェイプ**
    results = ray_coords[valid_mask.unsqueeze(
        -1).expand(-1, -1, 3)].view(-1, 3)
